...
# Prices per 1M tokens from the official OpenAI model pages:
# developers.openai.com/api/docs/models/gpt-6-{luna,sol,astra}
PRICES = {
...
def main():
    ...
    missing_routes = 0
    unpriced_actual_models, unpriced_jev_models = {}, {}
    included_turns = []
    for tk in all_turns:
        tok = tk["tok"]
        ...
        try:
            ...
            cj = None
            unpriced_jev_models[r] = unpriced_jev_models.get(r, 0) + 1
        try:
            ca = cost(tk["model"], tok)
        except KeyError:
            ca = None
            unpriced_actual_models[tk["model"]] = unpriced_actual_models.get(tk["model"], 0) + 1
        if ca is None or cj is None:
            skipped += 1
            continue
    ...
    comparison_complete = bool(included_turns) and skipped == 0
    comparison_status = (
        "complete_priced_sample" if comparison_complete
        else "partial_priced_sample" if included_turns
        else "unknown_no_comparable_priced_turns"
    )
    pct = (sum_actual - sum_jev) / sum_actual * 100 if comparison_complete and sum_actual else None
    ...
    result = {
        ...
        "savings_vs_astra_pct": (
            round((astra_all - sum_jev) / astra_all * 100, 1)
            if comparison_complete and astra_all else None
        ),
        "routes": {k: {"turns": dist_counts.get(k, 0),
                       "unpriced_turns": unpriced_jev_models.get(k, 0),
                       "cost_usd": round(dist_costs.get(k, 0), 2)}
                   for k in (*PRICES, "gpt-6-luna", "gpt-6-sol")},
        "routes_detail": routes,
        "actual_models": model_seen,
        "unpriced_actual_models": unpriced_actual_models,
        "unpriced_jev_turns": sum(unpriced_jev_models.values()),
        "unpriced_jev_models": unpriced_jev_models,
    ...
    qualifier = "" if comparison_complete else " (partial priced sample)"
    if included_turns:
        print(f"actual scenario  (session mix, published prices){qualifier}: ${sum_actual:.2f}")
        print(f"Jev scenario{qualifier}: ${sum_jev:.2f}")
    else:
        print("actual scenario: unknown (no comparable priced turns)")
        print("Jev scenario: unknown (no comparable priced turns)")
    if comparison_complete:
        print(f"→ ESTIMATED SAVINGS: {pct:.1f} %")
    elif included_turns:
        print(f"→ ESTIMATED SAVINGS: unknown (incomplete priced sample; {skipped} turns excluded)")
    else:
        print("→ ESTIMATED SAVINGS: unknown (no comparable priced turns)")
    ...
    if included_turns:
        print("100% scenarios: " + " · ".join(
            ...
            + (" (partial priced sample)" if not comparison_complete else ""))
        print(f"full-Astra baseline: ${astra_all:.0f} (fixed token volume; not measured quota)"
              + (" — partial priced sample" if not comparison_complete else ""))
    else:
        print("100% scenarios and full-Astra baseline: unknown (no comparable priced turns)")
    print("-" * 72)
    for k in ("gpt-6-luna", "gpt-6-sol", "gpt-6-astra", "gpt-5.6-luna", "gpt-5.6-sol"):
        unpriced = unpriced_jev_models.get(k, 0)
        print(f"  {k:<14} {dist_counts.get(k,0):>4} priced turns · "
              f"{unpriced:>4} unpriced · ${dist_costs.get(k,0):.2f}")

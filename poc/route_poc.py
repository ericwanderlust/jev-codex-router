...
Key lookup: JEV_ENV_FILE, JEV_API_KEY_FILE, ~/.hermes/.env, ~/.jev.env, then process env.
"""
...
def load_key():
    ...
    paths = [os.environ.get("JEV_ENV_FILE", "").strip()]
    for path in paths:
        if not path:
            continue
        ...
        if os.path.exists(p):
            with open(p, encoding="utf-8") as env_file:
                lines = [line.strip() for line in env_file]
    ...
    if key_file:
        try:
            with open(key_file, encoding="utf-8") as key_handle:
                value = key_handle.read().strip().strip("'\"")
            if value:
                return value
        except OSError:
            pass
            ...
            for name in ("JEV_API_KEY", "TYPESAFE_API_KEY"):
                for line in lines:
                    if line.startswith(f"{name}="):
                        ...
                        if v:
                            return v
    return (os.environ.get("JEV_API_KEY") or os.environ.get("TYPESAFE_API_KEY", "")).strip().strip("'\"")
...
def run(args):
    ...
    if not args.dry and not key:
        print("!! Jev API key not found (~/.hermes/.env, ~/.jev.env, process environment). Use --dry to validate payloads.")
        return 2
    q = questions()
    results = []
    for t in tasks:

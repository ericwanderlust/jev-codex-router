...
idle and reads no credential. A later explicit
`codex chatgpt-session enable` action may read only Codex's own `auth.json` and
records separate consent; provider credentials, Keychain, and other CLI
sessions remain undiscovered. See [docs/INSTALL.md](docs/INSTALL.md#credential-free-idle-install).

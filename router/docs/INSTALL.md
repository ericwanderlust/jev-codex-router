...
- Provider credential files are not scanned; only the protected credential
  bound to an explicitly enabled generic provider at a literal loopback
  address may be checked. The macOS Keychain and other CLIs' OAuth and session
  files are never read. Codex's own `auth.json` stays unread without one of
  the explicit native permissions described below.
...
- Without native session sharing, the merged catalog publishes no native
  account models. Only the explicitly configured loopback routes may appear.
- A clean idle install with no later loopback route returns a local
  `503 router_idle_no_provider`. After a user explicitly enables a loopback
  route, only that route can forward unless a native permission below applies.
  Router listeners remain on `127.0.0.1`; the local bridge may still contact
  its documented upstream.

`./bin/control codex-caller-auth enable` grants only permission to read Codex's
`auth.json` to compare the bearer already supplied on an incoming request.
A matching caller may relay that same bearer to native endpoints. This does
not enable fallback credentials, account catalog/cache reads, or Jev's native
second hop. The permission defaults off; use `status` or `disable` to inspect
or revoke it. Enabling records a credential-free marker without reading auth.

The separate `./bin/control chatgpt-session enable` permission allows local
router-authenticated clients, including Jev, to use the Codex session as a
fallback and permits the fixed ChatGPT account catalog plus its local cache.
General provider/Keychain discovery stays off under either permission.

...
The key comes from `JEV_API_KEY` or `TYPESAFE_API_KEY`, then `~/.hermes/.env` —
the same sources the local router uses. Read it, never print it. Pass `askUrl` to send the
...
- On `no-api-key`, tell the user which file needs `JEV_API_KEY` or
  `TYPESAFE_API_KEY` — never ask

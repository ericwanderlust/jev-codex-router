...
//
// The key comes from JEV_API_KEY or TYPESAFE_API_KEY (process environment,
// then the same env
...
/** JEV_API_KEY or TYPESAFE_API_KEY from the environment or router env files. */
...
export async function askJev({ state, questions }, options = {}) {
  ...
  if (askUrl) {
    ...
    if (!key) throw new Error("no Jev API key: set JEV_API_KEY or TYPESAFE_API_KEY in the environment or ~/.hermes/.env");
  ...
  try {
    ...
    const reason =
      error?.name === "AbortError"
        ? "timeout"
        : message.startsWith("no Jev API key") || message.startsWith("no TypeSafe key")
          ? "no-api-key"
          : "request-failed";
    return {
      status: "error",

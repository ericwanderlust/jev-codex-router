...
import { nativeSessionAccessAllowed } from "./codex-native-session.mjs";
...
function captureNative(cache) {
  // Native consent permits the dedicated account cache. Keep CLI account
  // probing disabled under no-discovery; the fixed catalog transport alone
  // refreshes that cache without discovering other credentials.
  const idle = discoveryDisabled();
  const routedActive = routedCatalogActive();
  const resolved = cache ?? (nativeSessionAccessAllowed() ? readModelsCache() : {});
...
function nativeCatalog({ refreshNative = refresh } = {}) {
  ...
  // Account cache access has the same explicit native consent boundary.
  const cache = nativeSessionAccessAllowed() ? readModelsCache() : {};

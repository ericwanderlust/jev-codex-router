...
export function codexAuthStatus() {
  ...
  // only explicitly shared Codex session data may establish native access.
  if (discoveryDisabled()) {
    const authenticated = nativeSessionAvailable();
    return { authenticated, reason: authenticated ? "native-session-consented" : "discovery-disabled" };
  }
  const binary = findCodexBinary();

...
import { CODEX_HOME, NATIVE_SESSION_CONSENT_PATH, NATIVE_CALLER_AUTH_CONSENT_PATH } from "./paths.mjs";
...
export function nativeCallerAuthEnabled() {
  try {
    const parsed = JSON.parse(readFileSync(NATIVE_CALLER_AUTH_CONSENT_PATH, "utf8"));
    return parsed?.version === 1 && parsed.caller_auth === "enabled";
  } catch {
    return false;
  }
}

export function setNativeCallerAuthEnabled(enabled) {
  if (enabled) {
    writePrivateJson(NATIVE_CALLER_AUTH_CONSENT_PATH,
      { version: 1, caller_auth: "enabled" }, { directoryMode: 0o700 });
  } else {
    rmSync(NATIVE_CALLER_AUTH_CONSENT_PATH, { force: true });
  }
...
export function nativeSessionSharingEnabled() {
  // --no-discovery still blocks the session unless the user explicitly saved
  // the separate sharing-consent marker. That marker authorizes only Codex's
  // own auth.json, not provider, Keychain, or other CLI credential discovery.
  const consented = consentMarkerEnabled();
  if (discoveryDisabled() && !consented) return false;
  const override = process.env.CODEX_ROUTER_NATIVE_SESSION_FALLBACK;
  if (override !== undefined) return override === "1" && (!discoveryDisabled() || consented);
  return consented;
}

// General discovery remains disabled; the explicit consent opens only the
// Codex native session, its fixed account catalog, and native request paths.
export function nativeSessionAccessAllowed({
  discoveryOff = discoveryDisabled,
  sharingEnabled = nativeSessionSharingEnabled,
} = {}) {
  return !discoveryOff() || sharingEnabled();
}
...
function readAuthDocument({ explicitSessionSharing = false, callerAuthentication = false } = {}) {
  // Caller-only consent permits this read solely for bearer comparison. It
  // does not authorize metadata/catalog reads or fallback session headers.
  if (discoveryDisabled() && !explicitSessionSharing && !nativeSessionSharingEnabled()
      && !(callerAuthentication && nativeCallerAuthEnabled())) {
    return undefined;
  }
...
function readSession(options) {
  return sessionFromAuthDocument(readAuthDocument(options));
}
...
export async function nativeAccountCatalogHeaders() {
  if (!nativeSessionAccessAllowed()) return undefined;
...
export function nativeSessionTokenMatches(token) {
  ...
  const auth = readAuthDocument({ callerAuthentication: true });
...
// A caller-only grant can relay exactly the credential already on this request.
// Requests from Jev/harness with only a local capability still require sharing.
export function nativeRequestAccessAllowed(token) {
  return nativeSessionAccessAllowed() || nativeSessionTokenMatches(token);
}
...
export function setNativeSessionSharingEnabled(enabled) {
  ...
  const session = readSession({ explicitSessionSharing: true });
...
export function nativeSessionStatus() {
  ...
  // Without explicit sharing consent, no-discovery hides even auth.json's
  // presence and age. Once consent exists, only this Codex-owned session may
  // be read to report whether the authorized route is usable.
  if (discoveryDisabled() && !sharingEnabled) {
    return {

...
} else if (args[0] === "codex-caller-auth") {
  const action = args[1] || "status";
  if (args.length > 2 || !["status", "enable", "disable"].includes(action)) {
    throw new Error("Usage: control codex-caller-auth status|enable|disable");
  }
  const { nativeCallerAuthEnabled, setNativeCallerAuthEnabled } = await import("./codex-native-session.mjs");
  if (action !== "status") setNativeCallerAuthEnabled(action === "enable");
  process.stdout.write(`${JSON.stringify({ caller_auth_enabled: nativeCallerAuthEnabled() })}\n`);

...
// CLIs' session files, OAuth credential files, and account-aware Codex probes.
// An install made with `--no-discovery` promises none of those happen except
// the router-managed credential bound to an explicitly enabled loopback
// generic provider; that narrow readiness check is not credential discovery.
// The separate explicit ChatGPT-session consent may read only Codex's own
// auth.json.
// The promise has to survive the installer process:

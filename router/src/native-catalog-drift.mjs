...
export function nativeCatalogDriftDetected() {
  // Account-cache drift is visible only after explicit native consent when
  // discovery is disabled; other installs use the same existing source.
  const nativeAccess = nativeSessionAccessAllowed();
  ...
  try {
    if (!nativeAccess) {
      const parsed = existsSync(NATIVE_CATALOG_PATH)

...
import {
  isLoopbackGenericProviderEndpoint,
...
function explicitlyEnabledLoopbackProvider(providerId) {
  try {
    return readGenericProviders({ reservedProviderIds: PROVIDERS }).some(
      (provider) => (
...
export function resolveGenericProviderCredentialReference(providerId, secretRef) {
  ...
  if (
    ...
    (discoveryDisabled() && !explicitlyEnabledLoopbackProvider(providerId))
  ) {
    return undefined;
  }
  return configuredProviderFileCredential(provider);

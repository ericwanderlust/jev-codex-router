...
import { isLoopbackGenericProviderEndpoint } from "./generic-provider-state.mjs";
...
// A deliberate loopback bridge is different from discovering credentials or
// remote providers. In no-discovery mode, inspect readiness only for these
// explicitly registered, enabled routes; never touch other credential stores.
export function explicitLoopbackGenericProviderIds(
  providers = [...RUNTIME_PROVIDERS.values()],
  isConfigured = genericProviderConfigured,
) {
  return providers
    .filter((provider) => (
      ...
      isLoopbackGenericProviderEndpoint(provider.baseUrl)
    ))
    .filter((provider) => isConfigured(provider.id))
...
export function configuredProviderIds() {
  // No-discovery suppresses every account, keychain, CLI, and remote-provider
  // probe. An explicitly enabled loopback bridge may check only its own local
  // router-managed credential so a deliberate local integration can publish.
  if (discoveryDisabled()) return explicitLoopbackGenericProviderIds();

...
async function handleResponses(request, response, requestUrl) {
  ...
  try {
    ...
    // always worked -- but no-discovery without native consent answers locally.
    if (!route && !nativeRequestAccessAllowed(bearerToken(request.headers.authorization))) {
...
async function handleNativeRequest(request, response, requestUrl, defaultModel) {
  ...
  try {
    ...
    // install without native consent refuses them locally.
    if (!nativeRequestAccessAllowed(bearerToken(request.headers.authorization))) {
      writeIdleNoProviderError(response);
      return;
    }
    const headers = nativeHeaders(request);

...
const {
  ...
  nativeSessionAccessAllowed,
  nativeCallerAuthEnabled,
  setNativeCallerAuthEnabled,
  nativeRequestAccessAllowed,
...
const { NATIVE_SESSION_CONSENT_PATH, NATIVE_CALLER_AUTH_CONSENT_PATH } = await import("../src/paths.mjs");
...
test.beforeEach(() => {
  clearAuth();
  setNativeCallerAuthEnabled(false);
...
test("no-discovery permits only explicitly consented Codex session sharing", async () => {
  writeAuth({ access_token: ACCESS, account_id: ACCOUNT });
  process.env.CODEX_ROUTER_NO_DISCOVERY = "1";
  try {
    assert.equal(nativeSessionStatus().present, false);
    assert.equal(nativeSessionHeaders(), undefined);
    assert.equal(nativeSessionAccessAllowed(), false);
    assert.equal(nativeSessionTokenMatches(ACCESS), false);
    assert.equal(codexAuthStatus().authenticated, false);
    assert.equal(await nativeAccountCatalogHeaders(), undefined);
    process.env.CODEX_ROUTER_NATIVE_SESSION_FALLBACK = "1";
    assert.equal(nativeSessionAccessAllowed(), false, "an environment flag cannot replace consent");
    setNativeSessionSharingEnabled(true);
    ...
    assert.equal(nativeSessionAccessAllowed(), true);
    assert.equal(nativeSessionTokenMatches(ACCESS), true);
    assert.equal(nativeSessionTokenMatches("wrong-token"), false);
    assert.deepEqual(codexAuthStatus(), { authenticated: true, reason: "native-session-consented" });
    assert.deepEqual(await nativeAccountCatalogHeaders(), nativeSessionHeaders());
...
test("caller-only consent validates the inbound bearer without sharing or account discovery", async () => {
  process.env.CODEX_ROUTER_NO_DISCOVERY = "1";
  try {
    assert.equal(nativeCallerAuthEnabled(), false);
    assert.equal(nativeRequestAccessAllowed(ACCESS), false);
    setNativeCallerAuthEnabled(true);
    assert.equal(existsSync(authPath), false, "granting caller permission neither reads nor creates auth");
    assert.equal(nativeRequestAccessAllowed(ACCESS), false);
    writeAuth({ access_token: ACCESS, account_id: ACCOUNT });
    assert.equal(nativeSessionTokenMatches(ACCESS), true);
    assert.equal(nativeRequestAccessAllowed(ACCESS), true);
    assert.equal(nativeRequestAccessAllowed("wrong-token"), false);
    assert.equal(nativeRequestAccessAllowed(undefined), false, "Jev caller capability alone is insufficient");
    assert.equal(nativeSessionSharingEnabled(), false);
    assert.equal(nativeSessionHeaders(), undefined);
    assert.equal(nativeSessionStatus().present, false);
    assert.equal(codexAuthStatus().authenticated, false);
    assert.equal(await nativeAccountCatalogHeaders(), undefined);
    const { refreshNativeAccountCatalog } = await import("../src/native-account-catalog.mjs");
    const forbidden = () => { throw new Error("caller-only consent reached account discovery"); };
    assert.deepEqual(await refreshNativeAccountCatalog({
      cachePath: new Proxy({}, { get: forbidden }), lock: forbidden,
      versionProvider: forbidden, headersProvider: forbidden, fetchImpl: forbidden,
    }), { status: "disabled" });
    if (process.platform !== "win32") {
      assert.equal(statSync(NATIVE_CALLER_AUTH_CONSENT_PATH).mode & 0o777, 0o600);
    }
    assert.doesNotMatch(readFileSync(NATIVE_CALLER_AUTH_CONSENT_PATH, "utf8"), /sk-test|acct-/);
    writeFileSync(NATIVE_CALLER_AUTH_CONSENT_PATH, '{"version":2,"caller_auth":"enabled"}');
    assert.equal(nativeRequestAccessAllowed(ACCESS), false, "unknown marker fails closed");
    setNativeCallerAuthEnabled(true);
    setNativeCallerAuthEnabled(false);
    assert.equal(nativeRequestAccessAllowed(ACCESS), false, "revocation is immediate");
  } finally {
    delete process.env.CODEX_ROUTER_NO_DISCOVERY;
    setNativeCallerAuthEnabled(false);
  }

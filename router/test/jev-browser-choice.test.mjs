...
test("readApiKey accepts either Jev key name and gives JEV_API_KEY priority", async () => {
  ...
  try {
    ...
    delete process.env.JEV_API_KEY;
    delete process.env.TYPESAFE_API_KEY;
    if (savedJev !== undefined) process.env.JEV_API_KEY = savedJev;
...
test("chooseElement says so when no key is configured, without calling anything", async () => {
  ...
  try {
    ...
    assert.match(decision.detail, /JEV_API_KEY/);
  } finally {
    if (savedJev !== undefined) process.env.JEV_API_KEY = savedJev;
    if (savedTypesafe !== undefined) process.env.TYPESAFE_API_KEY = savedTypesafe;
  }

...
test("explicit native consent permits only the fixed account catalog under no-discovery", () =>
  withCache(async (cachePath) => {
    let calls = 0;
    const result = await refreshNativeAccountCatalog({
      cachePath, force: true, version: "0.153.2", headersProvider,
      discoveryOff: () => true, sharingEnabled: () => true, lock: noLock,
      fetchImpl: async (url, init) => {
        calls += 1;
    ...
    let consented = true;
    const revoked = await refreshNativeAccountCatalog({
      cachePath, discoveryOff: () => true, sharingEnabled: () => consented,
      lock: (operation) => { consented = false; return operation(); },
      fetchImpl: () => { throw new Error("revoked consent reached transport"); },
    });
    assert.deepEqual(revoked, { status: "disabled" });
  }));

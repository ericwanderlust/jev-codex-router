...
test("drift detection triggers republish with new arbitrary native in merged output", async () => {
  ...
  try {
    ...
    try {
      ...
      writeFileSync(NATIVE_SESSION_CONSENT_PATH, JSON.stringify({ version: 1, sharing: "enabled" }));
      assert.equal(nativeCatalogDriftDetected(), true, "native consent makes account-cache updates visible");
      rmSync(NATIVE_SESSION_CONSENT_PATH);

      const fakeCodex = path.join(tempDir, "codex-version-only");
      writeFileSync(
        fakeCodex,
        '#!/bin/sh\n[ "$1" = "--version" ] && printf "codex-cli 9.9.9\\n"\n',
      );
      chmodSync(fakeCodex, 0o755);
      process.env.CODEX_BIN = fakeCodex;
      writeFileSync(NATIVE_CATALOG_PATH, JSON.stringify({
        captured_with: "codex-cli 1.0.0",
        native_binary_fingerprint: "old-build",
        models: [oldNative],
      }));
      assert.equal(
        nativeCatalogDriftDetected(),
        true,
        "discovery-disabled drift detection still catches a local Codex update",
      );
    } finally {

...
const {
  ...
  disableProvider,
  enableProvider,
  explicitLoopbackGenericProviderIds,
...
test("no-discovery checks only explicitly enabled loopback generic routes", () => {
  const checked = [];
  const ids = explicitLoopbackGenericProviderIds(
    [
      {
        id: "jev",
        generic: true,
      ...
      {
        id: "untrusted-private",
        generic: true,
        enabled: true,
        allowPrivate: true,
        baseUrl: "http://192.168.1.20:4319/v1",
      },
      {
        id: "disabled-loopback",
        generic: true,
        enabled: false,
        allowPrivate: true,
        baseUrl: "http://localhost:4319/v1",
      },
      {
        id: "native",
        generic: false,
  ...
  assert.deepEqual(ids, ["jev"]);
  assert.deepEqual(checked, ["jev"]);
});

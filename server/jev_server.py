...
Display: ordinary assistant text replies begin with the actual model ID and
reasoning effort; streamed reasoning summaries also get an exact route tag
appended in place ( · 🧠 gpt-6-sol:low · ) when Codex displays them. A local
sentinel can hide the reply header. JSON-constrained responses stay unchanged.
...
# Route headers are visible by default; this sentinel hides them. Presentation
# metadata is removed from replayed history, including legacy trailing signatures.
HIDE_SIGNATURE_PATH = os.path.join(STATE, "jev-router.hide-signature")
...
def load_key():
    """Read the configured Jev key without copying it into the router config."""
    ...
    for path in (ENV_PATH, os.path.join(HOME, ".jev.env")):
        try:
            ...
            continue
    return (os.environ.get("JEV_API_KEY") or os.environ.get("TYPESAFE_API_KEY", "")).strip()
...
def warn_if_key_missing():
    ...
    if load_key():
        return False
    print(
        "[jev-router] warning: no Jev API key is configured; "
...
def _debug_shape(payload):
    ...
    key = caller_secret().encode("utf-8")
    ...
    return {
        ...
        "tools": names,
        "cache_prefix": {
            name: fingerprint(payload.get(name))
            for name in ("instructions", "tools", "reasoning", "text",
                         "tool_choice", "parallel_tool_calls", "include")
        } | {
            "input_items": [fingerprint(item) for item in items[:512]],
            "input_items_truncated": len(items) > 512,
        },
    }
...
def route_marker(model, effort):
    ...
    """
    _short, glyph = route_label(model)
    return f" · {glyph} {model or '?'}" + (f":{effort}" if effort else "") + " · "


def answer_signature(shown):
    """Leading actual model/reasoning-effort label for assistant text messages."""
    if os.path.exists(HIDE_SIGNATURE_PATH):
        return None
    model = shown.get("model") or "?"
    _short, glyph = route_label(model)
    effort = shown.get("effort") or "unspecified"
    return f"**{glyph} {model} · reasoning: {effort}**\n\n"
...
HEADER_RX = re.compile(
    r"\A\*\*(?:⚡|🧠|🚀|🌍|🐳|✨) [A-Za-z0-9._/-]+ · (?:thinking|reasoning): "
    r"(?:low|medium|high|xhigh|max|unspecified|non spécifié)\*\*\r?\n\r?\n")
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _ask(self):
        ...
        key = load_key()
        if not key:
            return self._json(503, {"error": {"message": "Jev API key is not configured"}})
        t0 = time.time()
        try:
    ...
    def _post(self):
        ...
        if not no_fallback:
            ...
            if debug:
                try:
                    append_private(os.path.join(STATE, "jev-router-debug.jsonl"), json.dumps(
                        {"at": time.strftime("%Y-%m-%dT%H:%M:%S"), "phase": "forward",
                         "cache_scope": scope, "shape": _debug_shape(payload)},
                        ensure_ascii=False) + "\n")

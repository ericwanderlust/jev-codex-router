...
class AnswerHeader(unittest.TestCase):
    ...
    def test_structured_json_skips_header_while_plain_text_shows_it(self):
        shown = {"model": jev.SOL, "effort": "low"}
        with mock.patch.object(jev.os.path, "exists", return_value=False):
            self.assertEqual(
                jev.presentation_signature({"text": {"format": {"type": "text"}}}, shown),
                self.HEADER,
            )
            for payload in (
                ...
                with self.subTest(payload=payload):
                    self.assertIsNone(jev.presentation_signature(payload, shown))

    def test_old_opt_in_headers_are_removed_from_replayed_history(self):
        payload = {"input": [{"role": "assistant", "content": self.OLD_HEADER + "old answer"}]}
        self.assertEqual(jev.strip_signatures(payload), 1)
        self.assertEqual(payload["input"][0]["content"], "old answer")

    def test_unknown_effort_header_is_removed_from_replayed_history(self):
        header = "**🚀 gpt-6-astra · reasoning: unspecified**\n\n"

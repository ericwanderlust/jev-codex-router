...
class KeyLoading(unittest.TestCase):
    def write_env(self, path, value, name="TYPESAFE_API_KEY"):
    ...
    def test_existing_jev_api_key_is_read_without_copying_it(self):
        with tempfile.TemporaryDirectory() as tmp:
            env_file = os.path.join(tmp, "existing.env")
            self.write_env(env_file, "existing-key", "JEV_API_KEY")
            with mock.patch.object(jev, "ENV_PATH", os.path.join(tmp, "missing.env")), \
                 mock.patch.object(jev, "HOME", tmp), \
                 mock.patch.dict(os.environ, {"JEV_ENV_FILE": env_file}, clear=True):
                self.assertEqual(jev.load_key(), "existing-key")

    def test_jev_api_key_wins_over_legacy_alias_regardless_of_line_order(self):
        with tempfile.TemporaryDirectory() as tmp:
            env_file = os.path.join(tmp, "existing.env")
            with open(env_file, "w", encoding="utf-8") as fh:
                fh.write("TYPESAFE_API_KEY=legacy-key\nJEV_API_KEY=current-key\n")
            with mock.patch.object(jev, "ENV_PATH", os.path.join(tmp, "missing.env")), \
                 mock.patch.object(jev, "HOME", tmp), \
                 mock.patch.dict(os.environ, {"JEV_ENV_FILE": env_file}, clear=True):
                self.assertEqual(jev.load_key(), "current-key")

    def test_raw_jev_api_key_file_can_be_reused_without_copying_into_env(self):
        with tempfile.TemporaryDirectory() as tmp:
            key_file = os.path.join(tmp, "typesafe-key")
            with open(key_file, "w", encoding="utf-8") as fh:
                fh.write("file-key\n")
            with mock.patch.object(jev, "ENV_PATH", os.path.join(tmp, "missing.env")), \
                 mock.patch.object(jev, "HOME", tmp), \
                 mock.patch.dict(os.environ, {
                     "JEV_ENV_FILE": "", "JEV_API_KEY_FILE": key_file,
                     "JEV_API_KEY": "stale-process-key",
                 }, clear=True):
    ...
    def test_process_environment_remains_the_last_fallback(self):
        with tempfile.TemporaryDirectory() as tmp:
            with mock.patch.object(jev, "ENV_PATH", os.path.join(tmp, "missing.env")), \
                 ...
                 mock.patch.dict(
                     os.environ,
                     {"JEV_ENV_FILE": "", "JEV_API_KEY": "process-jev-key",
                      "TYPESAFE_API_KEY": "process-key"},
                     clear=True,
                ):
                self.assertEqual(jev.load_key(), "process-jev-key")
    ...
    def test_missing_key_emits_one_actionable_startup_warning(self):
        ...
        self.assertIn("no Jev API key is configured", stderr.getvalue())

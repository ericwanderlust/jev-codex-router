...
class CacheScope(unittest.TestCase):
    ...
    def test_debug_fingerprints_change_without_exposing_prompt_text(self):
        first = payload_for([message("user", "private first message")])
        second = payload_for([message("user", "private second message")])
        with mock.patch.object(jev, "caller_secret", return_value="fixture-secret"):
            a = jev._debug_shape(first)["cache_prefix"]
            b = jev._debug_shape(second)["cache_prefix"]
        self.assertEqual(a["instructions"], b["instructions"])
        self.assertEqual(a["tools"], b["tools"])
        self.assertNotEqual(a["input_items"], b["input_items"])
        self.assertNotIn("private", json.dumps(a) + json.dumps(b))
...
class PerCallEndToEnd(unittest.TestCase):
    def setUp(self):
        ...
        for name in ("OFF_PATH", "SHADOW_PATH", "DEBUG_PATH", "HIDE_SIGNATURE_PATH",
    ...
    def test_sol_and_luna_effort_preserve_request_prefix(self):
        history = [message("user", "continue the task")]
        with mock.patch.object(
            jev, "call_jev_routed",
            side_effect=[answer(jev.SOL, "high"), answer(jev.LUNA, "medium")],
        ):
            for model, effort in ((jev.SOL, "high"), (jev.LUNA, "medium")):
                sent = payload_for(history, reasoning={"effort": "low"})
                self.call(sent)
                forwarded = Edge.payloads[-1]
    ...
    def test_tool_continuation_keeps_request_level_effort(self):
        history = [
            message("user", "inspect the code"),
            tool_call("c0"),
            tool_step("c0", "done"),
        ]
        sent = payload_for(history, reasoning={"effort": "low"})
        with mock.patch.object(jev, "call_jev_routed",
                               return_value=answer(jev.SOL, "high")):
            self.call(sent)
        forwarded = Edge.payloads[-1]

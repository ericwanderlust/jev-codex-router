...
class Usage(unittest.TestCase):
    ...
    def test_reasoning_is_not_billed_twice(self):
        ...
        self.assertEqual(report.token_credits(report.LEGACY_SOL, usage), 78.0)
        self.assertEqual(report.token_credits(report.LEGACY_LUNA, usage), 4.4)
        self.assertIsNone(report.token_credits(j.SOL, usage))
        self.assertIsNone(report.token_credits(j.LUNA, usage))
    ...
    def test_retries_count_and_external_fallback_does_not_inflate_native_savings(self):
        ...
        entries = [{"attempts": [
            {"model": report.LEGACY_LUNA, "speed": "default", "status": 500, "usage": usage},
            {"model": report.LEGACY_SOL, "speed": "default", "status": 200, "usage": usage},
        ...
        self.assertEqual(result["all_sol_baseline_model"], report.LEGACY_SOL)
        self.assertEqual(result["all_astra_credits"], 500.0)

    def test_current_and_historical_fast_calls_keep_their_api_surcharge(self):
        short_context = {"input": 1_000_000, "cached": 0, "output": 0}
        self.assertEqual(report.turn_cost(j.LUNA, mix=short_context), 0.1)
        self.assertEqual(report.turn_cost(j.SOL, mix=short_context), 2.0)
        self.assertEqual(report.turn_cost(j.LUNA), 0.1)
        self.assertEqual(report.turn_cost(j.SOL), 2.0)
        for model in (j.LUNA, j.SOL, report.LEGACY_LUNA, report.LEGACY_TERRA,
                      report.LEGACY_SOL, j.ASTRA):
    ...
    def test_report_cost_is_only_a_synthetic_rate_index(self):
        from datetime import datetime
        entry = {"_at": datetime(2026, 9, 24), "model": j.LUNA, "tier": "luna"}
        result = report.summarize(
            [entry], 7,
            {"lines": 1, "out_of_window": 0, "undated": 0, "unparsable": 0},
            "local-log", backtest_path="/missing-local-backtest.json",
        )
        self.assertEqual(result["cost"]["units_per_reference_call"][j.LUNA], 1.0)
        self.assertEqual(result["cost"]["routed_rate_index_units"], 1.0)
        self.assertNotIn("savings_vs_astra_pct", result["cost"])
        self.assertEqual(result["measured_usage"]["legacy_calls_without_attempts"], 1)
